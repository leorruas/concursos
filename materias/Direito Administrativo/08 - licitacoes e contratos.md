...
```mermaid
graph TD
    A["Contratação Direta Sem Licitação"] --> B{"É possível haver competição?"}
    B -- Sim --> C["Dispensa de Licitação"]
    B -- Não --> D["Inexigibilidade de Licitação"]
    C --> E["A lei autoriza a não realização (ex: emergência de enchente, baixo valor)"]
    D --> F["A competição é inviável (ex: artista consagrado, fornecedor exclusivo)"]
```
...
```mermaid
graph LR
    P["1. Preparatória"] --> D["2. Divulgação Edital"]
    D --> A["3. Propostas/Lances"]
    A --> J["4. Julgamento"]
    J --> H["5. Habilitação"]
    H --> R["6. Fase Recursal"]
    R --> HO["7. Homologação"]
