...
- **Sistema Operacional de Ingestão Assistida (`scripts/ingest-vault.js`)**:
  - Toda ingestão de novos conteúdos da inbox deve ser executada ou validada por meio de `scripts/ingest-vault.js`.
  - **Modo Padrão**: `--dry-run` (simulação completa sem gravação em disco).
  - **Modo Efetivo**: `--apply` (requer confirmação explícita do plano de alteração e opera com transação reversível/rollback).
  - **Comandos canônicos**:
  ...
  - **Proteção Rígida de Fontes Brutas**: O script bloqueia imediatamente qualquer tentativa de escrita em `2 - Editais/`, notas marcadas como "NÃO MEXER" e referências imutáveis.
  - **Classificação Ambigua**: Interrompe o fluxo e gera relatório de pendências se a categoria da entrada for duvidosa.
- **Relatórios**: Documentos com dados quantitativos devem declarar o período de coleta/extração no início.
- **Housekeeping**: Deletar scripts temporários imediatamente após o uso. Não deletar o arquivo de ingestão (`00 inbox/00 ingestão.md`), apenas limpar o seu conteúdo mantendo o arquivo com o seu título.
